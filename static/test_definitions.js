...
export const tests = async () => {
  ...
  return {
    ...
    SharedWorker: {
      ...
      write: async (secret) => {
        try {
          const worker = new SharedWorker('supercookies_sharedworker.js');
          worker.port.start();
    ...
    CacheStorage: {
      ...
      write: async (key) => {
        try {
          const cache = await caches.open('supercookies');
...
*/
    ...
    'Web SQL Database': {
      ...
      write: async (key) => {
        ...
        const database = window.openDatabase('sqlite_supercookie', '', 'supercookie', 1024 * 1024);
        await new Promise((resolve) => database.transaction(tx => {
          tx.executeSql(
      ...
      read: async () => {
        const database = window.openDatabase('sqlite_supercookie', '', 'supercookie', 1024 * 1024);
        const result = await new Promise((resolve, reject) => database.transaction(tx => {
          tx.executeSql(
